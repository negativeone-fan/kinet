...
def KINET_DSMC(x, v, rand_vec, training=False):
    ...
    n, chnl = x.shape
    device = x.device 
    bs, _, _ = rand_vec.shape
    rand_vec = rand_vec.to(device)
    new_n = int(n / bs)

    v = v.reshape(bs, chnl, new_n)
    x = x.reshape(bs, chnl, new_n)
    ...
    delta_v = torch.zeros((bs, chnl, new_n, new_n))
    ...
    eye_matrix = torch.eye(new_n, device=collision_mask.device).unsqueeze(0)  # shape: (1, n, n)
